...
:::tip 提示
这篇教程适合对喜欢边学边做或小程序开发完全没有了解的开发者，如果你倾向于按部就班地学习，请把 [文档](./docs/README) 的内容全部过一遍。当你看完 [文档](./docs/README) 就可以发现：本篇教程就是文档的渐近式索引。
:::

这篇教程将分为四个章节：

- [环境准备](./guide#环境准备)：当我们使用 Taro 需要安装什么东西；
- [基础教程](./guide#基础教程)：Taro 的基础概念和开发指南；
- [项目优化](./guide#项目优化)：当项目变大变慢时，应该如何保持或提高应用的可维护性和性能；
- [多端开发](./guide#多端开发)：已经使用 Taro 开发了一端的应用，如何快速拓展到多端应用。
...
:::info 了解更多
除了 CSS 预处理器之外，Taro 还支持 [CSS Modules]() 和 [CSS-in-JS](./css-in-js)。
原理上还支持更多 CSS 工具，我们将在 [自定义编译](./guide/#自定义编译与插件化系统) 继续讨论这个问题。
:::
