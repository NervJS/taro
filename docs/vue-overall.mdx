...
[onLoad](./vue#onload-options) 之后，页面组件渲染到 Taro 的虚拟 DOM 之前触发。
...
但此时不代表 Taro 的虚拟 DOM 数据已经完成从逻辑层 `setData` 到视图层。因此这时**无法通过 `createSelectorQuery` 等方法获取小程序渲染层 DOM 节点。** 只能在 [onReady](./vue#onready-) 生命周期中获取。
