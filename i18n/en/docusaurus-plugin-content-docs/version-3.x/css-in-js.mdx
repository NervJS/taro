...
There is a common CSS-in-JS solution in the React community: [styled-components](https://github.com/styled-components/styled-components)。Unfortunately, however, `styled-components` uses the `<style>` tag to dynamically control styles, and there is no similar solution in applets. But we can achieve the same functionality with [linaria](https://github.com/callstack/linaria), `linaria` mainly provides the following features.
