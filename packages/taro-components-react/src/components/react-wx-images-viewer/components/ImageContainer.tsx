...
export interface ImageContainerProps {
  maxZoomNum: number
  handleStart: () => void
  handleMove: (diff: number) => void
  handleEnd: (isAllowChange: boolean) => boolean
  src: string
  screenWidth: number
  screenHeight: number
  left: number
 ...
 * d: duration（持续时间）。
 */
function easeOutQuart (t: number, b: number, _c: number, d: number) {
  const c = _c - b
  return -c * ((t = t / d - 1) * t * t * t - 1) + b
}

function setScope (value: number, min: number, max: number) {
  if (value < min) {
    return min
  }
  if (value > max) {
...
class ImageContainer extends PureComponent<ImageContainerProps, State> {
  actualHeight = 0 // 图片实际高度
  actualWith = 0 // 图片实际宽度

  originHeight = 0 // 图片默认展示模式下高度
  originWidth = 0 // 图片默认展示模式下宽度
  originScale = 1 // 图片初始缩放比例
  originTop: number

  startLeft = 0 // 开始触摸操作时的 left 值
  startTop = 0 // 开始触摸操作时的 top 值
  startScale = 1 // 开始缩放操作时的 scale 值
  startX: number
  startY: number

  onTouchStartTime = 0 // 单指触摸开始时间

  isTwoFingerMode = false // 是否为双指模式
  oldPointLeft = 0 // 计算手指中间点在图片上的位置（坐标值）
  oldPointTop = 0 // 计算手指中间点在图片上的位置（坐标值）
  _touchZoomDistanceStart = 0 // 用于记录双指距离
  _touchZoomDistanceEnd = 0 // 用于记录双指距离
  haveCallMoveFn = false
  isCalledHandleStart: boolean

  diffX = 0 // 记录最后 move 事件 移动距离
  diffY = 0 // 记录最后 move 事件 移动距离
  ...
  handlePopState = () => {
    window.removeEventListener('popstate', this.handlePopState)
    this.props.onError({ errMsg: 'previewImage:fail cancel' })
    this.props.onClose()
  }
