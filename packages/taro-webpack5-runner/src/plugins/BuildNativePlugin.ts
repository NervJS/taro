...
export default class BuildNativePlugin extends MiniPlugin {
  ...
  addLoadChunksPlugin (compiler: Compiler) {
    ...
    // 共享运行时下 taro chunk 已被 externals 关闭（见 MiniCombination getOptimization)、不再产出 taro.js；
    // 若仍在每个 native-component chunk 顶部 require('../../taro') 运行时会崩，故过滤掉 taro chunk 的
    // require 引用（runtime/vendors/common 仍保留）。
