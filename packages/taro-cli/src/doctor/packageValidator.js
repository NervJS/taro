...
const isCliVersionNotMatch = _.compose(_.negate(_.equals(cliPkg.version)), _.get('installed'))
...
function taroCliVersionNotMatch (pkgs) {
  ...
  const lines = _.map(pkg => Object({
    desc: `${pkg.moduleName} (${pkg.installed}) 与当前使用的 @tarojs/cli (${cliPkg.version}) 版本不一致, 请更新为统一的版本`,
    valid: false
  }), pkgsNotMatch)
  return lines
}
...
function taroShouldUpdate (pkgs) {
  ...
  return [{
    ...
    desc: `检测到最新稳定版本 Taro ${taroPkg.latest} , 当前 cli 版本 ${cliPkg.version}`,
