...
function makeSureReactNativeInstalled (workspaceRoot: string): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    ...
    if (checkReactNativeDependencies(packageInfo)) {
      ...
      let packages = `react@^18.1.0 react-dom@^18.1.0 react-native@^0.70.4 expo@~47.0.0-beta.8 @tarojs/taro-rn${devTag} @tarojs/components-rn${devTag} @tarojs/rn-runner${devTag}`
