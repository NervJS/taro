...
function bindEvents (weappComponentConf, events, isPage) {
  let target = weappComponentConf
  if (!isPage) {
    weappComponentConf.methods = weappComponentConf.methods || {}
    target = weappComponentConf.methods
  }
  events.forEach(name => processEvent(name, target))
}
    ...
    if (component['$$refs'] && component['$$refs'].length > 0) {
      ...
      component['$$refs'].forEach(ref => {
        ...
        if (ref.type === 'component') {
          ...
          const query = jd.createSelectorQuery().in(component.$scope)
...
function createComponent (ComponentClass, isPage) {
  ...
  if (isPage) {
    ...
    __jdRoute && cacheDataSet(__jdRoute, ComponentClass)
  } else {
