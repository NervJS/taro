...
const deviceInfo = get('NativePackageManager').loadLibrarySync('@ohos.deviceInfo', [])
...
export const getWindowInfo: typeof Taro.getWindowInfo = () => {
  return {
    ...
    safeArea: safeArea,
    foldDisplayMode: screen.foldDisplayMode,
    isFoldable: screen.isFoldable,
    heightBreakpoint: screen.heightBreakpoint,
    widthBreakpoint: screen.widthBreakpoint,
  }
}
...
export const getAppBaseInfo: typeof Taro.getAppBaseInfo = () => {
  const info: ReturnType<typeof Taro.getAppBaseInfo> = {
    ...
    // Note: 更新配置时才能记录
    theme: screen.theme // 来自cpp层系统注入的全局变量
  }
...
const isReadyPromise = get('WindowManager').isReadyPromise()
/* 异步版本 */
