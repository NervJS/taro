...
import { MethodHandler, temporarilyNotSupport, validateParams } from '../utils'
...
(Current as any).contextPromise.then(async (ctx) => {
  context = ctx

  try {
    // 创建KVManager实例
    kvManager = await get('NativePackageManager').loadLibrary('@ohos.data.distributedKVStore', [['createKVManager', {
      context: context,
      bundleName: 'com.example.taro'
    }]])
    // 继续创建获取数据库
    const data = await get('NativePackageManager').loadLibrary('@ohos.bundle.bundleManager', [[
      'getBundleInfoForSelfSync',
      await get('NativePackageManager').loadLibrary('@ohos.bundle.bundleManager', [['BundleFlag'], ['GET_BUNDLE_INFO_WITH_APPLICATION']])
    ]])

    const kvStoreType = await get('NativePackageManager').loadLibrary('@ohos.data.distributedKVStore', [['KVStoreType'], ['SINGLE_VERSION']])
    const securityLevel = await get('NativePackageManager').loadLibrary('@ohos.data.distributedKVStore', [['SecurityLevel'], ['S1']])
    kvStorePromise = new Promise(resolve => {
      kvManager.getKVStore<distributedKVStore.SingleKVStore>(`${data.appInfo.uid}Store`, {
