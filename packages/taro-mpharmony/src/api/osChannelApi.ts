...
try {
  // @ts-ignore
  bundleManager.getBundleInfoForSelf(bundleManager.BundleFlag.GET_BUNDLE_INFO_WITH_SIGNATURE_INFO | bundleManager.BundleFlag.GET_BUNDLE_INFO_WITH_APPLICATION).then(bundleInfoFor => {
...
const errMsgMap = new Map([[401, 'Parameter error'], [201, 'Permission denied'], [3, 'URL using bad/illegal format or missing URL'], [7, 'Couldn\'t connect to server'], [23, 'Failed writing received data to disk/application'], [25, 'Upload failed'], [26, 'Failed to open/read local data from file/application'], [28, 'Timeout was reached'], [73, 'Remote file already exists'], [78, 'Remote file not found'], [999, 'Unknown Other Error']])
...
class RequestTask {
  ...
  constructor (object: any) {
    ...
    this.httpRequest.request(typeof url === 'string' ? url : '', httpRequestOptions).then(data => {
