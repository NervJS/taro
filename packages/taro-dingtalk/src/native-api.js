...
const nativeRequest = dd.canIUse('request') ? dd.request : dd.httpRequest
  ...
  Object.keys(weApis).forEach(key => {
    if (!onAndSyncApis[key] && !noPromiseApis[key]) {
      taro[key] = (options, ...args) => {
        ...
        if (!(newKey in dd)) {
          console.warn(`钉钉小程序暂不支持 ${newKey}`)
          return
        }
        if (typeof options === 'string') {
          if (args.length) {
            return dd[newKey](options, ...args)
          }
          return dd[newKey](options)
        }
      ...
      taro[key] = (...args) => {
        ...
        if (key === 'getStorageSync') {
          const arg1 = args[0]
          if (arg1 != null) {
            return dd[key]({ key: arg1 }).data || dd[key]({ key: arg1 }).APDataStorage || ''
          }
        ...
        return dd[key].apply(dd, newArgs)
      }
    }
  })
}
