...
const pageExtraFns = ['onTitleClick', 'onOptionMenuClick', 'onPageScroll', 'onPullDownRefresh', 'onReachBottom', 'onShareAppMessage', 'onKeyboardShow', 'onKeyboardHide']
...
export function componentTrigger (component, key, args) {
    ...
    if (component['$$refs'] && component['$$refs'].length > 0) {
      ...
      component['$$refs'].forEach(ref => {
        ...
        if (ref.type === 'component') {
          ...
          const query = dd.createSelectorQuery().in(component.$scope)
  ...
  if (key === 'componentWillUnmount') {
    if (component.$scope.props) {
      ...
      if (compid) dd.propsManager.delete(compid)
    }
  }
  ...
  if (isPage) {
    ...
    Object.assign(weappComponentConf, {
      didMount () {
        ...
        const props = filterProps(ComponentClass.defaultProps, dd.propsManager.map[compid], {})
        ...
        if (compid) {
          dd.propsManager.observers[compid] = {
