...
export const createWxmlVisitor = (
  ...
  const jsxAttrVisitor = (path: NodePath<t.JSXAttribute>) => {
    updateLogFileContent(
      `INFO [taroize] createWxmlVisitor - 解析JSXAttribute ${getLineBreak()}${path} ${getLineBreak()}`
    )
  ...
  const renameJSXKey = (path: NodePath<t.JSXIdentifier>) => {
    updateLogFileContent(
      `INFO [taroize] createWxmlVisitor - 解析JSXIdentifier ${getLineBreak()}${path} ${getLineBreak()}`
    )
  ...
  return {
    ...
    Identifier: {
      enter (path: NodePath<t.Identifier>) {
        updateLogFileContent(
          `INFO [taroize] createWxmlVisitor - 解析Identifier ${getLineBreak()}${path} ${getLineBreak()}`
        )
    ...
    JSXElement: {
      enter (path: NodePath<t.JSXElement>) {
        updateLogFileContent(
          `INFO [taroize] createWxmlVisitor - 解析JSXElement ${getLineBreak()}${path} ${getLineBreak()}`
        )
        const openingElement = path.get('openingElement')
        const jsxName = openingElement.get('name')
        ...
        if (tagName === 'Template') {
          ...
          if (template) {
            ...
            traverse(ast, {
              ...
              BlockStatement (path) {
                updateLogFileContent(
                  `INFO [taroize] createWxmlVisitor - 解析BlockStatement ${getLineBreak()}${path} ${getLineBreak()}`
                )
                if (funcs.size > 0) {
