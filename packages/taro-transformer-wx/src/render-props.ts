...
export function injectRenderPropsListener(
  attrPath: NodePath<t.JSXAttribute>,
  attrName: string,
  attrExpr: t.ArrowFunctionExpression,
  ...
  block.children = [t.jSXExpressionContainer(t.callExpression(t.identifier(renderClosureFuncName), [renderPropsArgs]))]
...
function setRenderPropsEvents(attrPath: NodePath<t.JSXAttribute>, renderClosureFuncName: string) {
  ...
  if (classDecl && classDecl.isClassDeclaration()) {
    ...
    for (const s of classDecl.node.body.body) {
      if (t.isClassProperty(s) && (s.key as any).name === RENDER_PROPS_EVENTS && t.isArrayExpression(s.value)) {
        hasEvent = true
    ...
    if (!hasEvent) {
      classDecl.node.body.body.push(
        t.classProperty(t.identifier(RENDER_PROPS_EVENTS), t.arrayExpression([t.stringLiteral(renderClosureFuncName)]))
      )
    }
  }
}
...
export function injectRenderPropsEmiter(callExpr: NodePath<t.CallExpression>, attrName: string) {
  ...
  if (Array.isArray(callExpr.node.arguments) && callExpr.node.arguments.length) {
    args.push(callExpr.node.arguments[0] as t.Identifier)
  }
...
function buildListener(renderClosureFuncName: string, renderPropsArgs: t.MemberExpression) {
  return t.expressionStatement(
    t.callExpression(t.memberExpression(buildEventCenterMemberExpr(), t.identifier('on')), [
      t.stringLiteral(renderClosureFuncName),
      t.arrowFunctionExpression(
        ...
        t.blockStatement([
          t.ifStatement(
            t.unaryExpression(
              '!',
              t.callExpression(t.memberExpression(t.identifier('Taro'), t.identifier('shallowEqual')), [
                t.identifier('e'),
                renderPropsArgs,
              ])
            ),
