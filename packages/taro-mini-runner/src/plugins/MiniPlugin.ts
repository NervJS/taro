...
import { generateQuickAppManifest, generateQuickAppScriptContent, getTaroJsQuickAppComponentsPath } from '../utils/helper'
...
export default class TaroMiniPlugin {
  ...
  generateQuickAppTemplateFile (compilation: webpack.compilation.Compilation, componentName: string, templStr: string) {
    ...
    let baseTemplInfo: {
      ...
      taroSelfComponents: string[],
      templStr: string
    } | undefined

    if (componentName !== 'app') {
      let [templStr, taroSelfComponents] = this.options.template.buildTemplate(componentConfig) as any
    ...
    if (baseTemplInfo) {
      for (const item of baseTemplInfo.taroSelfComponents) {
        ...
        if (!compilation.assets[templatePath]) {
        ...
        const importPath = promoteRelativePath(path.relative(baseTemplInfo.fileTemplName, componentPath))
        baseTemplInfo.templStr = `<import name="${item}" src="${importPath}"></import>\n` + baseTemplInfo.templStr
      }
      let scriptContent = compilation.assets[`quickapp${this.options.fileType.script}`].source()
      scriptContent = generateQuickAppScriptContent(scriptContent)
