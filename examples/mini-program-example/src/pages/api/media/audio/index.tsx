    ...
    list: [
      ...
      },
      {
        ...
        func: (apiIndex) => {
          ...
          else if (this.state.innerConTextList.length === 1){ // 当innerConTextList数组只有一个音频实例，直接停止
            innercontext.seek(150)        
          }
          else { // 当innerConTextList数组不存在音频实例，向右移动一个指针，使用当前音频实例来停止
            this.setState ((preState : any)=>{ 
              return preState.innerConTextList = [...preState.innerConTextList,innercontext]
            })
            innercontext.seek(150)
      ...
      },
      {
        ...
        func: (apiIndex, data) => {
          ...
          else if (this.state.AudioList.length === 1){ // 当AudioList数组只有一个音频实例，直接跳转
            audioContext.seek(data.position)        
          }
          else {
            this.setState ((preState : any)=>{ // 当AudioList数组不存在音频实例，向右移动一个指针，使用当前音频实例来跳转
              return preState.AudioList = [...preState.AudioList,audioContext]
            })
            audioContext.seek(data.position)
          }
