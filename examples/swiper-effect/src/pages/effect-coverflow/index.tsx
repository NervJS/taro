...
export default function Index() {
  ...
  return (
    <View className='index'>
      ...
      {/* <SharePoster /> */}
      <View>底下是轮播图</View>
      <View className='coverflow-wrap'>
      <Swiper
        indicatorColor = '#000'
        indicatorActiveColor= '#fff'
        indicatorDots
        className='coverflow-swiper-wrap'
        current={current}
        circular = {circular}
        autoplay={autoplay}
        onChange={(e) => {
      ...
      >
        {
          arr.map((item, index) => {
            return (
              <SwiperItem key={index} itemId={String(index)} className='coverflow-item'>
                <Image className='coverflow-img' src={item.url}/>
              </SwiperItem>
            )
