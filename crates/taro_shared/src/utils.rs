...
pub async fn generate_with_template(from_path: &str, dest_path: &str, data: &impl serde::Serialize) -> anyhow::Result<()> {
  ...
  generate_with_template_content(&from_template, dest_path, data).await
}

pub async fn generate_with_template_content(template: &str, dest_path: &str, data: &impl serde::Serialize) -> anyhow::Result<()> {
