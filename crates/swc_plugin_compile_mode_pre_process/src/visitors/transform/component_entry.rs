...
impl<'a> ComponentEntryVisitor<'a> {
    ...
    pub fn get_format_render_fn_map_by_component_name(&self, component_name: &String) -> Option<HashMap<String, RenderFn>> {
        self.format_render_fn_map.get(component_name)
...
impl<'a> VisitMut for ComponentEntryVisitor<'a> {
                    ...
                    match (&decl.name, *arrow_expr.body) {
                        (Pat::Ident(ident), BlockStmtOrExpr::BlockStmt(_block_stmt)) => {
                            ...
                            if let Some(render_fn_map) = render_fn_map {
                                let mut process_visitor = TransformProcessVisitor::new(component_name, &render_fn_map);
    ...
    fn visit_mut_fn_decl(&mut self, n: &mut FnDecl) {
        ...
        if let Some(render_fn_map) = render_fn_map {
            let mut process_visitor = TransformProcessVisitor::new(component_name, &render_fn_map);
            n.function.body.visit_mut_children_with(&mut process_visitor);
        }
    }
        ...
        match &mut n.decl {
            ...
            DefaultDecl::Fn(fn_decl) => {
                ...
                if let Some(render_fn_map) = render_fn_map {
                    ...
                    fn_decl.function.body.visit_mut_children_with(&mut process_visitor);
                }
            },
            _ => {}
        }
