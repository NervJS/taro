...
async fn replace_name_in_utf8_file(file_path: &str, project_name: &str, default_name: &str) -> anyhow::Result<()> {
  ...
  let new_file_content_str = regex.replace_all(new_file_content_str.as_str(), project_name.to_lowercase());
...
async fn process_dot_files(file_path: &str) -> anyhow::Result<()> {
  let dot_file = UNDERSCORED_DOTFILES.iter().find(|&&e| file_path.contains(&format!("_{}", e)));
  if let Some(&dot_file) = dot_file {
    rename_file(file_path, &format!("_{}", dot_file), &format!(".{}", dot_file)).await?;
  }
  Ok(())
}

pub async fn change_default_name_in_template(project_name: &str, template_path: &str, project_path: &str) -> anyhow::Result<()> {
  ...
  let mut project_files = get_all_files_in_folder(project_path.to_string(), &["yarn.lock", "package-lock.json"])?;
  ...
  for file_path in project_files.iter() {
    ...
    if should_rename_file(file_path, &default_name.to_lowercase()) {
      rename_file(file_path, &default_name.to_lowercase(), &project_name.to_lowercase()).await?;
    }

    process_dot_files(file_path).await?;
  }
  println!("{}{}", style("✔").green(), style("项目名更新成功！").color256(238));
  Ok(())
}
