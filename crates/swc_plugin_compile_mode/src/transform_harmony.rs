                    ...
                    if *op == op!("&&") {
                        ...
                        fn get_element_double(condition: &mut Box<Expr>, right: &mut Box<Expr>) -> Expr {
                            Expr::Cond(CondExpr {
                                ...
                                cons: right.take(),
                                alt: Box::new(
                                    utils::create_self_closing_jsx_element_expr(
    ...
    fn build_ets_children(
        ...
        children.iter_mut().for_each(|child| {
            ...
            match child {
                ...
                JSXElementChild::JSXExprContainer(JSXExprContainer { expr: JSXExpr::Expr(jsx_expr), .. }) => {
                    // 如果套着 ()，则获取括号里面的内容
                ...
                JSXElementChild::JSXFragment(child_el) => {
                    self.pop_node_stack();
                    let (child_string, inner_retain) = self.build_ets_children(
                        &mut child_el.children,
                        Some(retain_child_counter)
                    );
                    children_string.push_str(&child_string);
                    if inner_retain != 0 {
            ...
            if let JSXAttrOrSpread::JSXAttr(jsx_attr) = attr {
                if let JSXAttrName::Ident(Ident { sym: name, .. }) = &jsx_attr.name {
                    ...
                    if jsx_attr_name == DIRECTION_ATTR {
                        ...
                        if
                            let Some(JSXAttrValue::JSXExprContainer(JSXExprContainer { expr, .. })) =
                                &mut jsx_attr.value
                        {
                            if let JSXExpr::Expr(expr) = expr {
                                            ...
                                            if let Prop::KeyValue(KeyValueProp { key, value, .. }) = &mut **prop {
                                                if let PropName::Ident(Ident { sym: name, .. }) = key {
                                                    ...
                                                    if name == "_flexDirection" {
                                                        ...
                                                        if
                                                            let Expr::Member(MemberExpr { obj, prop, .. }) =
                                                                &mut **value
                                                        {
                                                            if
                                                                let Expr::Ident(Ident { sym: obj_name, .. }) =
                                                                    &mut **obj
                                                            {
                                                                if
                                                                    let MemberProp::Ident(
                                                                        Ident { sym: prop_name, .. },
                                                                    ) = &mut *prop
                                                                {
                                                                    if
                                                                        obj_name == "FlexDirection" &&
                                                                        prop_name == "Column"
                                                                    {
...
impl VisitMut for TransformVisitor {
    ...
    fn visit_mut_jsx_element(&mut self, el: &mut JSXElement) {
        ...
        if self.is_compile_mode {
            ...
            let tmpl_main_contents = utils::add_spaces_to_lines(
