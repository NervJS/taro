  ...
  build() {
    Column() {
      if ((this.node0.childNodes[0] as TaroElement)._attrs.compileIf) {
        Column() {
          if (this.node0.childNodes[0].childNodes[0].nodeType === NodeType.TEXT_NODE) {
            if (this.node0.childNodes[0].childNodes[0].parentNode) {
              if ((this.node0.childNodes[0].childNodes[0].parentNode as TaroElement).tagName === 'BUTTON') {
                Text(this.node0.childNodes[0].childNodes[0].textContent)
                  .attributeModifier(textModify.setNode(this.node0.childNodes[0].childNodes[0]?.parentElement as TaroElement, {
                    fontSize: getButtonFontSize(this.node0.childNodes[0].childNodes[0].parentNode as TaroButtonElement),
                    color: getButtonColor(this.node0.childNodes[0].childNodes[0].parentNode as TaroButtonElement, BUTTON_THEME_COLOR.get((this.node0.childNodes[0].childNodes[0].parentNode as TaroButtonElement)._attrs.type || '').text)
                  }))
              } else {
                Text(this.node0.childNodes[0].childNodes[0].textContent)
                  .attributeModifier(textModify.setNode(this.node0.childNodes[0].childNodes[0]?.parentElement as TaroElement))
                  .width(getTextInViewWidth(this.node0.childNodes[0].childNodes[0].parentElement))
              }
            }
          } else {
            Text(this.node0.childNodes[0].childNodes[0].textContent) {
              // text 下还有标签
              if (this.node0.childNodes[0].childNodes[0].childNodes.length > 1 || ((this.node0.childNodes[0].childNodes[0].childNodes[0] && this.node0.childNodes[0].childNodes[0].childNodes[0] as TaroElement)?.nodeType === NodeType.ELEMENT_NODE)) {
                ForEach(this.node0.childNodes[0].childNodes[0].childNodes, (item: TaroElement) => {
                  createTextChildNode(item, getSpanVerticalAlign((this.node0.childNodes[0].childNodes[0] as TaroElement).hmStyle?.verticalAlign))
                }, (item: TaroElement) => item._nid)
              }
            }
            .onClick(shouldBindEvent((e: ClickEvent) => eventHandler(e, 'click', this.node0.childNodes[0].childNodes[0]  as TaroElement), this.node0.childNodes[0].childNodes[0]  as TaroElement, ['click']))
            .attributeModifier(textModify.setNode(this.node0.childNodes[0].childNodes[0] as TaroElement).withNormalStyle())
