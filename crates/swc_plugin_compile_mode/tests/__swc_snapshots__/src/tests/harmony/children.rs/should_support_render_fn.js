  ...
  build() {
    Column() {
      ...
      Column() {
        ...
        if (this.node2.childNodes[1].nodeType === NodeType.TEXT_NODE) {
          if (this.node2.childNodes[1].parentNode) {
            if ((this.node2.childNodes[1].parentNode as TaroElement).tagName === 'BUTTON') {
              Text(this.node2.childNodes[1].textContent)
                .attributeModifier(textModify.setNode(this.node2.childNodes[1]?.parentElement as TaroElement, {
                  fontSize: getButtonFontSize(this.node2.childNodes[1].parentNode as TaroButtonElement),
                  color: getButtonColor(this.node2.childNodes[1].parentNode as TaroButtonElement, BUTTON_THEME_COLOR.get((this.node2.childNodes[1].parentNode as TaroButtonElement)._attrs.type || '').text)
                }))
            } else {
              Text(this.node2.childNodes[1].textContent)
                .attributeModifier(textModify.setNode(this.node2.childNodes[1]?.parentElement as TaroElement))
                .width(getTextInViewWidth(this.node2.childNodes[1].parentElement))
            }
          }
        } else {
          Text(this.node2.childNodes[1].textContent) {
            // text 下还有标签
            if (this.node2.childNodes[1].childNodes.length > 1 || ((this.node2.childNodes[1].childNodes[0] && this.node2.childNodes[1].childNodes[0] as TaroElement)?.nodeType === NodeType.ELEMENT_NODE)) {
              ForEach(this.node2.childNodes[1].childNodes, (item: TaroElement) => {
                createTextChildNode(item, getSpanVerticalAlign((this.node2.childNodes[1] as TaroElement).hmStyle?.verticalAlign))
              }, (item: TaroElement) => item._nid)
            }
          }
          .onClick(shouldBindEvent((e: ClickEvent) => eventHandler(e, 'click', this.node2.childNodes[1]  as TaroElement), this.node2.childNodes[1]  as TaroElement, ['click']))
          .attributeModifier(textModify.setNode(this.node2.childNodes[1] as TaroElement).withNormalStyle())
